const ProfilePreviewSection = ({
  ...
  const teachSkills = Array.isArray(formData?.teachSkills)
    ? formData.teachSkills
    : [];
  const learnSkills = Array.isArray(formData?.learnSkills)
    ? formData.learnSkills
    : [];
  const availability = Array.isArray(formData?.availability)
    ? formData.availability
    : [];
  const learningGoals = Array.isArray(formData?.learningGoals)
    ? formData.learningGoals
    : [];
  const badges = Array.isArray(earnedBadges) ? earnedBadges : [];
    ...
    <section className="space-y-3">
      ...
      <div className="grid gap-4 rounded-2xl border border-white/10 bg-[#0E1620] p-4 md:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
        <div className="flex flex-col items-center justify-center rounded-xl border border-white/10 bg-[#0B1420] px-4 py-5 text-center">
          ...
          <p className="mt-3 text-lg font-semibold leading-tight text-[#DCE7F5]">
            {composeFullName(formData.firstName, formData.lastName) ||
              "Your Name"}
          </p>
          <p className="mt-1 text-sm text-[#8DA0BF]">
            @{formData.username || "username"}
          </p>
          <div className="mt-2 flex items-center gap-2 text-sm text-gray-300">
            <span className="text-yellow-400">⭐</span>
            <span>
              {profileRating.avgRating > 0
                ? profileRating.avgRating.toFixed(1)
                : "0.0"}
            </span>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-2">
            <div className="rounded-lg border border-white/10 bg-[#111721] px-3 py-2 text-center">
              <p className="text-[11px] uppercase tracking-wide text-[#8DA0BF]">
                Teaching Skills
              </p>
              <p className="mt-1 text-base font-semibold text-[#DCE7F5]">
                {teachSkills.filter((s) => s?.skillName?.trim()).length}
              </p>
            </div>
            <div className="rounded-lg border border-white/10 bg-[#111721] px-3 py-2 text-center">
              <p className="text-[11px] uppercase tracking-wide text-[#8DA0BF]">
              ...
              <p className="mt-1 text-base font-semibold text-[#DCE7F5]">
                {learnSkills.filter((s) => s?.skillName?.trim()).length}
              </p>
            </div>
            <div className="rounded-lg border border-white/10 bg-[#111721] px-3 py-2 text-center">
              <p className="text-[11px] uppercase tracking-wide text-[#8DA0BF]">
            ...
            <div className="rounded-lg border border-white/10 bg-[#111721] px-3 py-2 text-center">
              ...
              <p className="mt-1 text-base font-semibold text-[#DCE7F5]">
                {
                  learningGoals.filter((goal) => String(goal || "").trim())
                    .length
                }
              </p>
            </div>
          ...
          <div>
            <p className="text-center text-[11px] uppercase tracking-wide text-[#8DA0BF]">
              Achievements
            </p>
