...
const WhiteboardModal = lazy(
  () => import("../components/classroom/WhiteboardModal"),
);
...
const SwapClassroom = () => {
  ...
  const hasValidClassId = /^(?:\d+|[a-f\d]{24})$/i.test(classId);
  const { user } = useAuth();
    ...
    if (incrementSession) {
      setSwapClass((prev) => {
        ...
        const currentCount = Number(
          prev.sessionCount || prev.totalSessions || 0,
        );
        return {
          ...prev,
  ...
  const persistSharedNote = async (content) => {
    try {
      ...
      toast.error(
        error?.response?.data?.message || "Failed to save shared notes",
      );
    } finally {
  ...
  useEffect(() => {
    ...
    const fetchClassData = async () => {
      try {
        ...
        setSwapClass(data);
        setResources(
          Array.isArray(data?.pinnedResources) ? data.pinnedResources : [],
        );
        setSnippets(Array.isArray(data?.codeSnippets) ? data.codeSnippets : []);
        setClassroomFiles(
          Array.isArray(data?.classroomFiles) ? data.classroomFiles : [],
        );

        const noteContent = note?.content ?? data?.sharedNote?.content ?? "";
        const noteUpdated =
          note?.updatedAt ?? data?.sharedNote?.updatedAt ?? null;
        ...
        toast.error(
          error?.response?.data?.message || "Failed to load classroom",
        );
      } finally {
        setLoading(false);
      }
  ...
  }, [classId, clearChatUnread, hasValidClassId]);
  ...
  useEffect(() => {
    ...
    const handlePresence = ({ userIds = [] }) => {
      ...
      const partnerId =
        swapClass.swapRequest.fromUserId === user?.userId
    ...
    const handleCallPresence = async ({ userIds = [] }) => {
      ...
      const bothInCall =
        normalizedUserIds.includes(user?.userId) &&
        normalizedUserIds.includes(partnerId);
    ...
    const handleCallOffer = async ({ fromUserId, toUserId, sdp }) => {
      if (
        Number(toUserId) !== Number(user?.userId) ||
        Number(fromUserId) === Number(user?.userId)
      )
        return;
      if (!sdp) return;
    ...
    const handleSharedNoteUpdated = ({
      classId: updatedClassId,
      content,
      updatedAt,
      updatedBy,
    }) => {
      if (String(updatedClassId) !== String(classId)) return;
    ...
    const handleMessageReaction = ({ messageId, emoji, previousEmoji }) => {
      ...
      setMessageReactions((prev) =>
        applyReactionChange(prev, Number(messageId), emoji, previousEmoji),
      );
    };

    const handleWhiteboardSceneUpdated = ({
      classId: updatedClassId,
      scene,
    }) => {
      if (String(updatedClassId) !== String(classId) || !scene) return;
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      let created;
      if (selectedFile) {
        created = await sendAttachmentMessage(
          classId,
          selectedFile,
          newMessage.trim(),
        );
  ...
  const handleLoadOlder = async () => {
    if (!messagesMeta.hasMore || !messagesMeta.nextCursor || loadingOlder)
      return;
  ...
  const highlightedText = (text) => {
    ...
    return parts.map((part, idx) =>
      idx % 2 === 1 ? (
        <mark
          key={`${part}-${idx}`}
          className="rounded bg-[#0A4D9F]/40 px-0.5 text-[#DCE7F5]"
        >
          {part}
        </mark>
      ) : (
  ...
  const handleReactToMessage = (messageId, emoji) => {
    ...
    setMessageReactions((prev) =>
      applyReactionChange(prev, normalizedMessageId, nextEmoji, previousEmoji),
    );
  ...
  const handleSubmitReview = async () => {
    const missingCategory = REVIEW_CATEGORY_CONFIG.find(
      (category) => Number(reviewRatings[category.key] || 0) < 1,
    );
    if (missingCategory) {
      toast.error(
        `Please rate ${missingCategory.label.toLowerCase()} (1-5 stars)`,
      );
      return;
    }
  ...
  const handleHelpfulVote = async (reviewId) => {
    try {
      ...
      toast.error(
        error?.response?.data?.message || "Could not mark review as helpful",
      );
    }
  };
  ...
  const handlePreviewClassroomFile = (file) => {
    if (!isPreviewableFile(file?.fileName)) {
      toast(
        "Preview is not available for this file type. You can still download it.",
      );
      return;
    }
    setPreviewFile(file);
  ...
  const bothUsersOnline =
    classParticipants.length === 2 &&
    classParticipants.every((participant) =>
      onlineParticipantIds.includes(Number(participant.userId)),
    );
  ...
  const swapRequest = swapClass.swapRequest || {};
  const teachSkillName = swapRequest.teachSkill?.skill?.name || "TBD";
  const learnSkillName = swapRequest.learnSkill?.skill?.name || "TBD";
    ...
    <div className="bg-[#0A0F14]">
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        <ClassroomPanel
          ...
          actions={
            <span
              className={`rounded-full px-3 py-1 text-xs font-bold uppercase ${isFinished ? "bg-green-500/10 text-green-400" : "bg-blue-500/10 text-blue-400"}`}
            >
              {swapClass.status}
            </span>
          }
        >
          <div className="mb-6 flex flex-col justify-between gap-4 rounded-xl border border-white/10 bg-slate-900 p-4 lg:flex-row lg:items-center">
            <div>
              <p className="text-xl font-semibold text-white">
                Classroom #{swapClass.id}
              </p>
              <p className="mt-1 text-base font-semibold text-[#DCE7F5]">
                {teachSkillName} ⇄ {learnSkillName}
              </p>
            </div>
          ...
          <div className="mb-6 rounded-xl border border-white/10 bg-slate-900 p-4">
            <p className="mb-3 text-xs uppercase tracking-wide text-[#8DA0BF]">
              Participants
            </p>
            <div className="space-y-2">
              {classParticipants.map((participant) => {
                const participantOnline = onlineParticipantIds.includes(
                  Number(participant.userId),
                );
                return (
                  <div
                    key={participant.userId}
                    className="flex items-center justify-between rounded-lg border border-white/10 bg-[#111721] px-3 py-2"
                  >
                    ...
                    <div className="flex items-center gap-2 text-xs text-[#DCE7F5]">
                      <span
                        className={`h-2 w-2 rounded-full ${participantOnline ? "bg-green-500" : "bg-gray-500"}`}
                      />
          ...
          <div className="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
            <div className="rounded-xl border border-white/10 bg-slate-900 p-4 text-center transition hover:border-blue-500">
              ...
              <p className="text-2xl font-semibold text-[#E6EEF8]">
                {sessionCount}
              </p>
            </div>
            <div className="rounded-xl border border-white/10 bg-slate-900 p-4 text-center transition hover:border-blue-500">
          ...
          {!isFinished && (
            <div className="mt-5 flex flex-wrap gap-3">
              <Button onClick={handleCompleteClass} size="sm" variant="primary">
                Mark Complete
              </Button>
            </div>
          )}
          ...
          <div className="flex items-center justify-between p-1">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-400">
                Draw diagrams, explain concepts, and sketch ideas together.
              </p>
            </div>
            <button
              type="button"
