...
import {
  ...
  getCalendarEvents,
} from "../services/swapsPage.service";
...
const Swaps = () => {
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      ...
      toast.success(
        action === "ACCEPTED"
          ? "Request accepted! A class has been created."
          : action === "REJECTED"
            ? "Request rejected."
            : "Request cancelled.",
      );
      ...
      if (action === "ACCEPTED") {
        await queryClient.refetchQueries({ queryKey: ["swaps", "classes"] });
  ...
  const renderSwapDirectionBlock = ({
    teachSkill,
    learnSkill,
    ...
    <div className="mt-3 flex items-center justify-center gap-4">
      <div className="w-28 rounded-lg border border-white/10 bg-[#0F172A] px-3 py-2 text-center">
        <p className="truncate text-sm font-medium text-[#DCE7F5]">
          {teachSkill || "Not specified"}
        </p>
  ...
  const renderProfileHoverCard = (username) => {
    ...
    const availability = Array.isArray(profile?.availability)
      ? profile.availability
          .map((slot) => String(slot.dayOfWeek || "").slice(0, 3))
          .filter(Boolean)
          .slice(0, 4)
      : [];
          ...
          <>
            <div className="flex items-center gap-2">
              ...
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold text-[#DCE7F5]">
                  {fullName}
                </p>
            ...
            <p className="text-xs text-[#DCE7F5]">
              {teaches.length ? teaches.join(", ") : "Not listed"}
            </p>
  ...
  const isActiveClass = (swapClass) => {
    const status = String(swapClass?.status || "ONGOING").toUpperCase();
    return status !== "COMPLETED" && status !== "CANCELLED";
  };
  const ongoingClasses = activeClasses.filter(isActiveClass);
  const completedClasses = activeClasses.filter(
  ...
  const activeSwapsCount = ongoingClasses.length;
  const pendingRequestsCount = [...receivedRequests, ...sentRequests].filter(
    (r) => String(r.status || "").toUpperCase() === "PENDING",
  ).length;
  ...
  const renderRequestCard = (req, type) => {
    ...
    const requestStatus = String(req.status || "PENDING").toUpperCase();
    const requestId = req.id ?? req._id;
    const requestClassId = req.swapClass?.id ?? req.swapClass?._id;
    ...
    const requestTimelineStep = (() => {
      if (requestStatus === "PENDING") return 0;
      if (requestStatus === "REJECTED") return 0;
      if (requestStatus === "CANCELLED") return req.swapClass ? 2 : 0;
    ...
    const getSkillName = (userSkill) =>
      userSkill?.skill?.name || userSkill?.skillName || userSkill?.name;
    const teachSkillName = isReceived
      ? getSkillName(req.learnSkill)
      : getSkillName(req.teachSkill);
    const learnSkillName = isReceived
      ? getSkillName(req.teachSkill)
      : getSkillName(req.learnSkill);
    const nextSession = requestClassId
      ? nextSessionByClassId[requestClassId]
      : null;
    ...
    return (
      <div
        key={requestId}
        className="overflow-hidden rounded-2xl border border-white/10 bg-[#111721] shadow-[0_16px_40px_rgba(0,0,0,0.55)] transition duration-200 hover:-translate-y-1 hover:bg-[#151D27]"
      >
        <div className="p-4">
          {/* Header row */}
          <div className="flex items-start justify-between gap-3">
            <div
              ...
              onMouseLeave={() =>
                setHoveredUsername((prev) =>
                  prev === otherUsername ? null : prev,
                )
              }
            >
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <Link
                    to={`/u/${otherUsername}`}
                    className="font-semibold text-[#DCE7F5] hover:text-[#0A4D9F] transition-colors"
                  >
                    {otherUsername}
                  </Link>
                  <span className="text-xs text-[#6F83A3]">
                    {isReceived
                      ? "sent you a request"
                      : "received your request"}
                  </span>
                </div>
                <p className="mt-0.5 text-xs text-[#6F83A3]">
            ...
            <div className="flex items-center gap-2">
              ...
              {renderCardActionMenu({
                menuKey: `request-${requestId}`,
                userId: otherUserId,
                username: otherUsername,
                chatClassId: requestClassId,
              })}
            </div>
          </div>
          ...
          {req.message && (
            <div className="mt-3 flex items-start gap-2 rounded-lg border border-white/10 bg-[#0E1620] p-3">
              ...
              <p className="text-sm italic text-[#8DA0BF]">
          ...
          <div className="flex justify-end gap-2 border-t border-white/10 bg-[#0E1620] px-4 py-2.5">
            {isReceived ? (
                ...
                <Button
                  ...
                  onClick={() =>
                    openConfirm(
                      ...
                      "REJECTED",
                      "Reject Request",
                      `Are you sure you want to reject the swap request from @${otherUsername}?`,
                    )
                  }
                >
                ...
                <Button
                  ...
                  onClick={() =>
                    openConfirm(
                      ...
                      "ACCEPTED",
                      "Accept Request",
                      `Accept the swap request from @${otherUsername}? A classroom will be created for you both.`,
                    )
                  }
                >
              ...
              <Button
                size="sm"
                variant="danger"
                onClick={() => openCancelDialog(requestId, otherUsername)}
              >
        ...
        {requestStatus === "ACCEPTED" && !isReceived && (
        ...
        <div className="flex flex-wrap justify-end gap-2 border-t border-white/10 bg-[#0E1620] px-4 py-2.5">
          ...
          <Button
            ...
            onClick={() => handleOpenChat(requestClassId)}
  ...
  const renderClassCard = (cls) => {
    ...
    const partnerUser =
      cls.swapRequest?.fromUser?.userId === user?.userId
    ...
    const nextSession = nextSessionByClassId[classId] || null;

    const unreadForClass = chatUnreadByClass?.[String(classId)] || 0;
      ...
      >
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            >
              ...
              <div>
                ...
                <p className="mt-0.5 text-xs text-[#6F83A3]">Swap #{classId}</p>
              </div>
          ...
          {cls.status === "ONGOING" && cls.completion && (
            <div className="mt-3 rounded-lg border border-[#F59E0B]/25 bg-[#F59E0B]/10 p-3">
              <p className="text-xs font-medium text-[#F59E0B]">
                ...
                {
                  [
                    cls.completion.completedByUser1,
                    cls.completion.completedByUser2,
                  ].filter(Boolean).length
                }
                /2 users marked complete
              </p>
            </div>
          )}
          ...
          {isCompleted && (
            <div className="mt-3 rounded-lg border border-[#22C55E]/25 bg-[#22C55E]/10 p-3">
              <p className="text-xs font-medium text-[#22C55E]">
                Completed on{" "}
                {new Date(cls.completion.completedAt).toLocaleDateString(
                  "en-US",
        ...
        <div className="flex flex-wrap justify-end gap-2 border-t border-white/10 bg-[#0E1620] px-4 py-2.5">
          ...
          <Button
            ...
            variant="secondary"
            className="gap-1.5 bg-[#0F172A] border border-white/10 text-gray-200 hover:bg-[#1E293B]"
            onClick={() => handleOpenChat(classId)}
          >
            <MessageCircle className="h-4 w-4" />
            Chat
          </Button>
  ...
  return (
    <div className="page-shell">
      ...
      <SwapsControlPanel
        ...
        onLearnMore={() =>
          toast(
            "Use the actions menu on swap cards to block users when needed.",
          )
        }
      />
            ...
            {loadingReceived ? (
              ...
              renderTabEmptyState(
                ...
                statusFilter === "ALL"
                  ? [
                      {
                        label: "Find Matching Partners",
                        to: "/discover?sort=best-match&rating=4",
                        variant: "secondary",
                      },
                      {
                        label: "Create Swap Request",
                        to: "/swaps/new",
                        variant: "secondary",
                      },
                    ]
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                {filterRequests(receivedRequests).map((req) =>
                  ...
                  ? [
                      {
                        ...
                        to: "/discover?sort=best-match&rating=3",
                        variant: "secondary",
                      },
                      {
                        label: "Browse by Category",
                        to: "/discover?category=Programming",
                        variant: "secondary",
                      },
                    ]
                  : [
                ...
                if (filteredClasses.length === 0) {
                  if (statusFilter === "COMPLETED") {
                    return renderTabEmptyState(
                      ...
                      [
                        {
                          label: "Open Active Classes",
                          onClick: () => setStatusFilter("ONGOING"),
                          variant: "secondary",
                        },
                        {
                          label: "Find New Partners",
                          to: "/discover?sort=best-match",
                          variant: "secondary",
                        },
                      ],
                    );
