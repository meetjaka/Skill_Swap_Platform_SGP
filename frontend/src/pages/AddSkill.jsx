  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (!isCreatingNew) {
        ...
        if (found) {
          skillId = found.id ?? found._id;
        } else {
          toast.error(
            "Skill not found. Please select from list or valid option.",
          );
          setIsLoading(false);
          return;
        }
        ...
        skillId = created.id ?? created._id;
      }
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        {!isCreatingNew ? (
          <div>
            <select
              {...register("skillName", {
                required: !isCreatingNew ? "Please select a skill" : false,
              })}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <Input
                ...
                {...register("newSkillName", {
                  required: isCreatingNew ? "Skill name is required" : false,
                })}
                placeholder="e.g. UI Design"
              />
              {errors.newSkillName && (
                <p className="mt-1 text-sm text-red-400">
                  {errors.newSkillName.message}
                </p>
              )}
            </div>
            <div>
          ...
          <div className="space-y-4 border-t pt-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Optional: upload a demo video (max 50MB, MP4/MOV)
              </p>
              {isUploading && (
                <div className="mt-2">
                ...
                {...register("proofUrl", {
                  validate: (value) => {
                    ...
                    return (
                      ...
                      "Enter a valid URL (GitHub, Portfolio, YouTube, etc.)"
                    );
                  },
                })}
