...
import {
  ...
  removeSkill,
  uploadSkillDemo,
} from "../services/profilePage.service";
import {
  emptyTeachSkill,
  emptyLearnSkill,
  WEEK_DAYS,
...
const Profile = () => {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      try {
        ...
        const userSkills = Array.isArray(skillsData) ? skillsData : [];
        ...
        setExistingUserSkills(
          userSkills.map((item) => ({ id: item.id, type: item.type })),
        );
        ...
        setProfileRating({
          avgRating: Number(
            profileData?.headerStats?.rating ||
              profileData?.reputationMetrics?.averageRating ||
              0,
          ),
          reviewCount: Number(
            profileData?.headerStats?.reviewCount ||
              profileData?.reputationMetrics?.reviewCount ||
              0,
          ),
        });
        setEarnedBadges(
          Array.isArray(profileData?.badges) ? profileData.badges : [],
        );
        const selectedTeachingStyles = parseTeachingStyles(profile);
        const goals =
          Array.isArray(profileData.learningGoals) &&
          profileData.learningGoals.length
            ? profileData.learningGoals
                .map((goal) => String(goal.goalText || "").trim())
                .filter(Boolean)
            : [""];
          ...
          profilePrivacy: {
            showAvailability:
              typeof privacy.showAvailability === "boolean"
                ...
                : true,
            showPortfolio:
              typeof privacy.showPortfolio === "boolean"
  ...
  const updateField = (key, value) => {
    const normalizedValue =
      key === "username" ? String(value).trim().toLowerCase() : value;
    ...
    switch (key) {
      ...
      case "username":
        if (!value?.trim()) error = "Username is required";
          ...
          error = `At least ${USERNAME_MIN} characters`;
        else if (value.trim().length > USERNAME_MAX)
          error = `Max ${USERNAME_MAX} characters`;
        else if (!USERNAME_REGEX.test(value.trim()))
          error = "Use only lowercase letters, numbers, and underscore (_)";
        break;
      case "bio":
        if (value && value.replace(/<[^>]*>/g, "").length > BIO_MAX_LENGTH)
          error = `Bio exceeds ${BIO_MAX_LENGTH} characters`;
        break;
      case "teachingStyles":
        if (!Array.isArray(value) || value.length < 1)
          error = "Please select at least one teaching style.";
        break;
      case "timezone":
        if (!isValidTimeZone(value))
          error = "Please choose a valid timezone (IANA format).";
        break;
      case "githubLink":
      case "linkedinLink":
      case "portfolioLink":
  ...
  const hasValidationErrors = () => {
    ...
    errs.teachingStyles = validateField(
      "teachingStyles",
      formData.teachingStyles,
    );
  ...
  const addArrayItem = (arrayName, itemFactory) => {
    setFormData((prev) => ({
      ...prev,
      [arrayName]: [...prev[arrayName], itemFactory()],
    }));
  };
  ...
  const removeArrayItem = (arrayName, index) => {
    setFormData((prev) => {
      ...
      return {
        ...prev,
        [arrayName]: next.length
          ? next
          : [
              arrayName === "teachSkills"
                ? emptyTeachSkill()
                : arrayName === "learnSkills"
                  ? emptyLearnSkill()
                  : emptyAvailability(prev.timezone),
            ],
      };
    });
  };
  ...
  const toggleAvailabilityDay = (slotIndex, dayValue) => {
    setFormData((prev) => {
      ...
      const currentDays = sortDays(
        slot.days || (slot.dayOfWeek ? [slot.dayOfWeek] : []),
      );
    ...
    try {
      ...
      throw new Error(
        err?.response?.data?.message ||
          `Failed to create skill "${normalizedName}"`,
      );
    }
  };
  ...
  const submitAll = async ({ finalize = false } = {}) => {
    ...
    setSaving(true);
    try {
      const flattenedAvailability = flattenAvailabilitySlots(
      ...
      profilePayload.append(
        "teachingStyles",
        JSON.stringify(formData.teachingStyles),
      );
      profilePayload.append(
        "learningGoals",
        JSON.stringify(
          formData.learningGoals.filter((goal) => String(goal || "").trim()),
        ),
      );
      ...
      for (let i = 0; i < formData.teachSkills.length; i += 1) {
        ...
        try {
          const skillId =
            teachSkill.skillId || (await resolveSkillId(teachSkill.skillName));
          if (!skillId) {
          ...
          if (teachSkill.videoFile) {
            ...
            const uploaded = await uploadSkillDemo(
              teachSkill.videoFile,
              (progress) => {
                updateTeachSkillUpload(i, { uploadProgress: progress });
              },
            );
            videoUrl = uploaded.url;
            updateTeachSkillUpload(i, {
              isUploading: false,
              uploadProgress: 100,
              videoUrl,
            });
          ...
          console.error(
            "Failed to save teach skill:",
            teachSkill.skillName,
            err,
          );
          toast.error(
            `Failed to save skill "${teachSkill.skillName}": ${err?.response?.data?.message || err.message}`,
          );
          skillSaveErrors += 1;
          // Retain old id so cleanup doesn't delete an existing skill
          if (teachSkill.id) retainedSkillIds.add(teachSkill.id);
      ...
      for (const learnSkill of formData.learnSkills) {
        ...
        try {
          const skillId =
            learnSkill.skillId || (await resolveSkillId(learnSkill.skillName));
          if (!skillId) {
          ...
          console.error(
            "Failed to save learn skill:",
            learnSkill.skillName,
            err,
          );
          toast.error(
            `Failed to save skill "${learnSkill.skillName}": ${err?.response?.data?.message || err.message}`,
          );
          skillSaveErrors += 1;
          if (learnSkill.id) retainedSkillIds.add(learnSkill.id);
        }
      ...
      const removed = existingUserSkills.filter(
        (item) => !retainedSkillIds.has(item.id),
      );
      ...
      for (const item of removed) {
        try {
          ...
          removeFailures.push(
            err?.response?.data?.message ||
              `Failed to remove skill #${item.id}`,
          );
        }
      }
      ...
      if (skillSaveErrors > 0 || removeFailures.length > 0) {
        if (skillSaveErrors > 0) {
          toast.error(
            `${skillSaveErrors} skill(s) could not be saved. Please try again.`,
          );
        }
        ...
        if (removeFailures.length > 0) {
          ...
          const detailMessage = uniqueRemovalMessages[0]
            ? ` ${uniqueRemovalMessages[0]}`
            : "";
          toast.error(`${baseMessage}${detailMessage}`);
        }
      } else {
        toast.success(
          finalize ? "Profile setup saved successfully" : "Changes saved",
        );
      }
      ...
      if (isSetupMode && finalize) {
        ...
        const [freshProfile, freshSkills] = await Promise.all([
          getMyProfile(),
          getUserSkills(),
        ]);
        const refreshedSkills = Array.isArray(freshSkills) ? freshSkills : [];
        const profile = freshProfile.profile || {};
        setAvatarPreview(profile.avatarUrl || "");
        setProfileUserId(freshProfile.userId || null);
        setProfileRating({
          avgRating: Number(
            freshProfile?.headerStats?.rating ||
              freshProfile?.reputationMetrics?.averageRating ||
              0,
          ),
          reviewCount: Number(
            freshProfile?.headerStats?.reviewCount ||
              freshProfile?.reputationMetrics?.reviewCount ||
              0,
          ),
        });
        setEarnedBadges(
          Array.isArray(freshProfile?.badges) ? freshProfile.badges : [],
        );

        const freshTeach = refreshedSkills
        ...
        const freshLearn = refreshedSkills
        ...
        setExistingUserSkills(
          refreshedSkills.map((s) => ({ id: s.id, type: s.type })),
        );
        ...
        setFormData((prev) => ({
          ...
          avatarFile: null,
          teachingStyles: refreshedTeachingStyles.length
            ? refreshedTeachingStyles
            : prev.teachingStyles,
          learningGoals:
            Array.isArray(freshProfile.learningGoals) &&
            freshProfile.learningGoals.length
              ? freshProfile.learningGoals
                  .map((goal) => String(goal.goalText || "").trim())
                  .filter(Boolean)
              : [""],
      ...
      toast.error(
        error.response?.data?.message || "Failed to save profile setup",
      );
    } finally {
      setSaving(false);
    }
        ...
        <div className="space-y-6">
          <section className={`${profileCardClass} space-y-6`}>
            <div className="space-y-1">
              <h2 className="text-[21px] font-semibold text-[#DCE7F5]">
                Profile Information
              </h2>
              <p className="text-sm text-[#8DA0BF]">
                Set up your public identity and tell people what you want to
                learn.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
              ...
              >
                {avatarPreview ? (
                  ...
                  <div className="flex h-full w-full items-center justify-center text-xl font-semibold text-[#DCE7F5]">
                    {(formData.firstName || formData.username || "U")
                      .charAt(0)
                      .toUpperCase()}
                  </div>
                )}
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-[#8DA0BF]">
                  Upload profile photo (PNG or JPG, max 5MB).
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              <div>
                <input
                  className={`w-full border rounded px-3 py-2 ${touched.firstName && fieldErrors.firstName ? "border-red-500/50" : ""}`}
              ...
              <div>
                <input
                  className={`w-full border rounded px-3 py-2 ${touched.lastName && fieldErrors.lastName ? "border-red-500/50" : ""}`}
            ...
            <div>
              <input
                className={`w-full border rounded px-3 py-2 ${fieldErrors.username ? "border-red-500/50" : ""}`}
            ...
            <div>
              ...
              <p className="text-xs text-[#8DA0BF] mb-3">
                Tell others about your experience, skills, or learning
                interests.
              </p>
              {editorLoading && (
                <div
                  className="animate-pulse space-y-2 border rounded p-4"
                  style={{ height: 180 }}
                >
              ...
              >
                <Editor
                  ...
                  init={{
                    ...
                    content_style:
                      "body { font-family: ui-sans-serif, system-ui; font-size: 14px; }",
                  }}
                />
              </div>
              ...
              <p className="text-xs text-[#6F83A3] mt-1">
                {(formData.bio || "").replace(/<[^>]*>/g, "").length} /{" "}
                {BIO_MAX_LENGTH} characters
              </p>
            </div>
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  updateField("learningLanguage", e.target.value)
                }
              >
            ...
            <div>
              ...
              {formData.teachingStyles.length > 0 && (
                <div className="mt-3">
                  <p className="mb-2 text-xs uppercase tracking-wide text-[#8DA0BF]">
                    Selected Styles
                  </p>
        ...
        <div className="space-y-6">
          <section className={profileCardClass}>
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                type="button"
                onClick={() => addArrayItem("teachSkills", emptyTeachSkill)}
            ...
            <div className="space-y-4">
              {formData.teachSkills.map((skill, index) => (
                ...
                >
                    ...
                    <div className="space-y-1">
                      <div className="h-2 rounded bg-gray-200 overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${Math.min(100, skill.uploadProgress)}%`,
                          }}
                        />
                      </div>
                      <p className="text-xs text-gray-600">
                        {skill.isUploading ? (
                          `Uploading... ${Math.round(skill.uploadProgress)}%`
                        ) : (
                  ...
                  <button
                    type="button"
                    onClick={() => removeArrayItem("teachSkills", index)}
          ...
          <section className={profileCardClass}>
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                type="button"
                onClick={() => addArrayItem("learnSkills", emptyLearnSkill)}
                className="px-3 py-1 text-sm bg-blue-600 text-white rounded"
              >
                Add Learn Skill
              </button>
            ...
            <div className="space-y-4">
              {formData.learnSkills.map((skill, index) => (
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={() => removeArrayItem("learnSkills", index)}
                    className="text-sm text-red-400 hover:text-red-300"
                  >
                    Remove
                  </button>
        ...
        <section className={`${profileCardClass} flex flex-col gap-4`}>
          <div>
            ...
            <p className="text-sm text-gray-600">
              Let others know when you are open to swap sessions.
            </p>
          </div>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <label className="flex items-center gap-2 text-sm text-gray-700">
              <input
                ...
                onChange={(e) =>
                  updateField("emailRemindersEnabled", e.target.checked)
                }
              />
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                type="button"
                onClick={() =>
                  addArrayItem("availability", () =>
                    emptyAvailability(formData.timezone || "UTC"),
                  )
                }
                className="px-3 py-1 text-sm bg-blue-600 text-white rounded"
              >
                      ...
                      return (
                        ...
                        >
                          <input
                            ...
                            checked={isSelected}
                            onChange={() =>
                              toggleAvailabilityDay(index, day.value)
                            }
                            className="hidden"
                          />
                          {day.label}
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {saving
                ? "Saving..."
                : `Save ${PROFILE_STEPS.find((item) => item.id === step)?.label || "Step"}`}
            </button>
