...
export const updateRequestStatus = async (req, res, next) => {
  try {
    const rawRequestId = String(req.params.id || "").trim();
    const requestId = /^\d+$/.test(rawRequestId)
      ? Number(rawRequestId)
      : rawRequestId;
    const userId = req.user.userId;
    if (
...
const normalizeId = (value, fieldName) => {
  ...
  if ((!Number.isInteger(id) || id <= 0) && !/^[a-f\d]{24}$/i.test(id)) {
...
export const deletePinnedResource = async (req, res, next) => {
  try {
    ...
    const userId = req.user.userId;
    const result = await deletePinnedResourceService(
      userId,
      classId,
      resourceId,
    );
